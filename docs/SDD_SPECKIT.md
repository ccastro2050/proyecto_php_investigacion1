...
clarify NO crea un archivo: escribe dentro de 2_spec.md
    ## Clarificaciones
    C5 — DELETE, ¿físico o lógico? -> Físico: la tabla área de conocimiento no
...
4. **Es evaluable y comparable.** Los `.md` se leen y se califican, y los
   cursos comparten la misma estructura sobre la misma `investigacion_php_local` con
   stacks distintos.
...
- **La versión en curso:** [spec_kit/versiones/v1_area_conocimiento/](spec_kit/versiones/v1_area_conocimiento/2_spec.md)
...
`PATCH /api/area_conocimiento/{codigo}` con body parcial (campos opcionales):
...
```bash
curl http://localhost:8111/api/area_conocimiento        # total: 8
curl -i http://localhost:8111/api/area_conocimiento/PR999   # → 404
```
...
- [ ] modelos/AreaConocimiento.php (la entidad: 4 propiedades)
- [ ] peticiones/AreaConocimientoCrear.php (validación: todo obligatorio)
...
```markdown
### RF6 — Eliminar area_conocimiento
DELETE /api/area_conocimiento/{codigo} elimina el área de conocimiento.
...
|---|---|---|---|---|
| RF1 — listar | §2 `GET /api/area_conocimiento` | Fase 5 · controlador | §2 comando 2 | 1 y 2 |
...
7. En este repositorio: la [constitución](spec_kit/1_constitution.md), el
   ...
   [spec kit de la v1](spec_kit/versiones/v1_area_conocimiento/2_spec.md).
