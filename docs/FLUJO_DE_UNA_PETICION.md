...
| **La ruta** | `$ruta = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);` | `$ruta` vale `"/api/area_conocimiento"` |
...
```php
if ($ruta === '/api/area_conocimiento') {          // ¿pidieron la colección?
    if ($metodo === 'GET') {
...
```
1. index.php        captura  $metodo="POST", $ruta="/api/area_conocimiento", $body=[...]
2. index.php        compara  y llama $controlador->crear($body)
3. ControladorAreaConocimiento::crear
...
4. ServicioAreaConocimiento::crear
      construye el objeto del modelo (new AreaConocimiento(...))
      y se lo pasa al repositorio (por la interfaz)
5. RepositorioAreaConocimientoMariaDB::crear
      INSERT INTO area_conocimiento (...) VALUES (:codigo, :nombre, ...)  ← PDO preparado
...
```mermaid
flowchart TD
    A["1. php -S / php-fpm recibe<br/>el texto HTTP"] --> B["2. index.php encuentra<br/>POST /api/area_conocimiento<br/>en la tabla de rutas"]
...
```
1. index.php        captura  $metodo="GET", $ruta="/api/area_conocimiento/PR001"
2. index.php        recorta  el código con substr → "PR001"
                    y llama  $controlador->obtener("PR001")
3. ControladorAreaConocimiento::obtener   try { responder(200, servicio->obtener(...)) }
4. ServicioAreaConocimiento::obtener      pide al repositorio; si llega null → lanza
                                  NoEncontradoExcepcion (el catch la vuelve 404)
5. RepositorioAreaConocimientoMariaDB     SELECT ... WHERE codigo = :codigo
6. La fila vuelve convertida en objeto AreaConocimiento y sale como JSON
```
...
Invoke-RestMethod "http://localhost:8111/api/area_conocimiento"
Invoke-RestMethod "http://localhost:8111/api/area_conocimiento/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8111/api/area_conocimiento" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8111/api/area_conocimiento/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8111/api/area_conocimiento/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8111/api/area_conocimiento/PR009"
```
