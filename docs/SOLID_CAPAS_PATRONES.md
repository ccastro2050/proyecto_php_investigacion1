...
│ El SQL con PDO: traduce filas ↔ objetos AreaConocimiento.   │
│ NO conoce HTTP. NO decide negocio.                  │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM area_conocimiento WHERE codigo = :codigo
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  MariaDB — investigacion_php_local
          └───────┬───────┘
                  │
   y la respuesta hace el viaje DE VUELTA:
   fila → objeto AreaConocimiento (repositorio) → objeto (servicio)
...
|---|---|---|---|
| **Controlador** | HTTP: validar el body, códigos de estado, JSON | SQL y reglas de negocio | `controladores/ControladorAreaConocimiento.php` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `servicios/ServicioAreaConocimiento.php` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `repositorios/RepositorioAreaConocimientoMariaDB.php` |
...
```
CAPA 1: FRONT (:8110)   → solo pinta y llama la API; nunca toca la BD
CAPA 2: API   (:8111)   → solo JSON
...
Pregunta legítima: la carpeta de modelos (`modelos/AreaConocimiento.php`) existe en el
...
- El **modelo es LO QUE VIAJA entre estaciones**: el repositorio arma un
  `AreaConocimiento` desde la fila, el servicio lo razona, el controlador lo
...
lenguaje. En PHP el modelo es la clase con propiedades tipadas — el `gran_area` que
...
if ($ruta === '/api/area_conocimiento' && $metodo === 'GET') {
...
(`RepositorioAreaConocimientoPostgreSQL implements IRepositorioAreaConocimiento`) y ajustar el
...
class RepositorioAreaConocimientoPostgreSQL implements IRepositorioAreaConocimiento { /* … */ }

// ...y el ensamblador (ÚNICO archivo tocado) elegirá el motor:
$repositorio = $motor === 'postgres'
    ? new RepositorioAreaConocimientoPostgreSQL($dsn, $usuario, $clave)
    : new RepositorioAreaConocimientoMariaDB($dsn, $usuario, $clave);
```
...
`RepositorioAreaConocimientoMariaDB` son indistinguibles para el servicio — por eso la
...
{
    public function obtenerPorCodigo(string $codigo): ?AreaConocimiento
    {
...
interface IRepositorioAreaConocimiento
{
    ...
    public function obtenerPorCodigo(string $codigo): ?AreaConocimiento;
    public function crear(AreaConocimiento $area_conocimiento): bool;
...
**En la v1:** `ServicioAreaConocimiento` recibe **la interfaz** por constructor
(`private readonly IRepositorioAreaConocimiento $repositorio`); solo
...
class ServicioAreaConocimiento implements IServicioAreaConocimiento
{
    public function __construct(
        private readonly IRepositorioAreaConocimiento $repositorio,  // ← interfaz, no clase
    ) {
    }
}
...
match (true) {
    $ruta === '/api/area_conocimiento' && $metodo === 'GET' => $controlador->listar(),
    // …
};
```
...
```php
// El contrato (repositorios/IRepositorioAreaConocimiento.php):
public function obtenerPorCodigo(string $codigo): ?AreaConocimiento;

// ServicioAreaConocimiento lo usa SIN saber si detrás hay MariaDB o un array
...
{
    public function __construct(
        private readonly IRepositorioAreaConocimiento $repositorio,  // ← llega armado
    ) {
    }
}
```
...
function crearRepositorioAreaConocimiento(string $motor): IRepositorioAreaConocimiento
{
    return $motor === 'postgres'
        ? new RepositorioAreaConocimientoPostgreSQL($dsnPg, $usuario, $clave)
        : new RepositorioAreaConocimientoMariaDB($dsnMaria, $usuario, $clave);
}
...
(`RepositorioAreaConocimientoMariaDB`, `RepositorioFalsoEnMemoria` — y los motores
